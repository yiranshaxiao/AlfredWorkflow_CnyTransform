...
def _handle_int_part(num, cny_str, zero_before, is_head=False):
    ...
    if num_of_place < 4 and not is_head:
        zero_before += 1
    for i in range(num_of_place, 0, -1):
        number = int(num / (10 ** (i - 1))) % 10
        if number != 0:
            if zero_before > 0:
                cny_str += '零'
                zero_before = 0
...
def _handle_dec_part(decimal, integer, cny_str, zero_before=False):
    assert 0 < decimal < 100

    if zero_before > 0 and decimal > 0 and integer > 0:
        cny_str += '零'
    for i in range(2, 0, -1):
        number = int(decimal / (10 ** (i - 1)) % 10)
...
def transform(price):
    ...
    yi = int(integer / 100000000 % 10000)
    wan = int(integer / 10000 % 10000)
    ge = int(integer % 10000)
