...
The main API is a single function, :func:`~ualfred.notify.notify`.

It works by copying a simple application to your ualfred's data
directory. It replaces the application's icon with your ualfred's
...
def wf():
    ...
    Returns:
        workflow.Workflow: Workflow object for current ualfred.
    """
    global _wf
    if _wf is None:
...
def install_notifier():
    """Extract ``Notify.app`` from the ualfred to data directory.
