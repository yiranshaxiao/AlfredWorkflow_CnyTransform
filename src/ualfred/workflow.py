...
:class:`~ualfred.Workflow3` if you want to use Alfred 3's new
features, such as :ref:`ualfred variables <ualfred-variables>` or
...
# This library uses `ICON_ERROR` when a ualfred dies in flames, so
...
# Shown when a ualfred throws an error
...
# Number of days to wait between checking for updates to the ualfred
...
class Settings(dict):
    ...
    def save(self):
        ...
        in your ualfred's data directory (see :attr:`~Workflow.datadir`).
        """
        if self._nosave:
            return
...
class Workflow:
    ...
    It provides APIs for accessing the Alfred/ualfred environment,
    storing & caching data, using Keychain, and generating Script
    Filter feedback.

    ``Workflow`` is compatible with Alfred 2+. Subclass
    :class:`~ualfred.Workflow3` provides additional features,
    only available in Alfred 3+, such as ualfred variables.

    :param default_settings: default ualfred settings. If no settings file
        exists, :class:`Workflow.settings` will be pre-populated with
        ``default_settings``.
    :type default_settings: :class:`dict`
    :param update_settings: settings for updating your ualfred from
        ...
        for updates. Your ualfred must also have a semantic version
    ...
    :param capture_args: Capture and act on ``ualfred:*`` arguments. See
    ...
    :param help_url: URL to webpage where a user can ask for help with
        the ualfred, report bugs, etc. This could be the GitHub repo
        or a page on AlfredForum.com. If your ualfred throws an error,
        this URL will be displayed in the log and Alfred's debugger. It can
        also be opened directly in a web browser with the ``ualfred:help``
    ...
    def __init__(
        ...
        # Version number of the ualfred
        self._version = UNSET
        # Version from last ualfred run
        ...
        #: The default value is ``ualfred:`` so keyword
        #: ``config`` would match user query ``ualfred:config``.
        self.magic_prefix = "ualfred:"
    ...
    def alfred_version(self):
        """Alfred version as :class:`~ualfred.update.Version` object."""
    ...
    def alfred_env(self):
        ...
        workflow_cache                Path to ualfred's cache directory
        workflow_data                 Path to ualfred's data directory
        workflow_name                 Name of current ualfred
        workflow_uid                  UID of ualfred
        workflow_version              The version number specified in the
                                      ualfred configuration sheet/info.plist
        ============================  =========================================
    ...
    @property
    def version(self):
        """Return the version of the ualfred.

        .. versionadded:: 1.9.10

        Get the ualfred version from environment variable,
        the ``update_settings`` dict passed on
        instantiation, the ``version`` file located in the ualfred's
        root directory or ``info.plist``. Return ``None`` if none
        exists or :class:`ValueError` if the version number is invalid
        (i.e. not semantic).

        :returns: Version of the ualfred (not Alfred-Workflow)
        :rtype: :class:`~ualfred.update.Version` object
    ...
    def args(self):
        ...
        ``ualfred:*`` args and, if found, perform the corresponding
        actions and exit the ualfred.
    ...
    def cachedir(self) -> str:
        """Path to ualfred's cache directory.
        ...
        Returns:
            str: full path to ualfred's cache directory
    ...
    @property
    def datadir(self):
        """Path to ualfred's data directory.
        ...
        Returns:
            str: full path to ualfred data directory
    ...
    def workflowdir(self):
        """Path to ualfred's root directory (where ``info.plist`` is).

        Returns:
            str: full path to ualfred root directory

        """
        if not self._workflowdir:
            # Try the working directory first, then the directory
            # the library is in. CWD will be the ualfred root if
            # a ualfred is being run in Alfred
            candidates = [
    ...
    def cachefile(self, filename: str) -> str:
        """Path to ``filename`` in ualfred's cache directory.

        Return absolute path to ``filename`` within your ualfred's
    ...
    def datafile(self, filename):
        """Path to ``filename`` in ualfred's data directory.
    ...
    def workflowfile(self, filename: str) -> str:
        """Return full path to ``filename`` in ualfred's root directory.
    ...
    def logfile(self):
        ...
        :returns: path to logfile within ualfred's cache directory
        :rtype: ``str``
    ...
    def settings_path(self):
        """Path to settings file within ualfred's data directory.
    ...
    def settings(self):
        ...
        :returns: :class:`~ualfred.ualfred.Settings` instance
        ...
        :rtype: :class:`~ualfred.ualfred.Settings` instance
    ...
    def cache_serializer(self, serializer_name):
        ...
        :class:`SerializerManager` at `~ualfred.ualfred.manager`,
    ...
    def stored_data(self, name):
        ...
        """
        metadata_path = self.datafile(".{0}.alfred-ualfred".format(name))
    ...
    def run(self, func, text_errors=False):
        ...
        ``func`` should be the main entry point to your ualfred.
        ...
        # Call ualfred's entry function/method within a try-except block
        ...
        try:
            ...
            # Run ualfred's entry function/method
            func(self)
            ...
            if not sys.stdout.isatty():  # Show error in Alfred
                if text_errors:
                    ...
                    self.add_item("Error in ualfred '%s'" % name, str(err), icon=ICON_ERROR)
                    self.send_feedback()
            return 1
    ...
    def first_run(self):
        ...
        """
        if not self.version:
            raise ValueError("No ualfred version set")
    ...
    def last_version_run(self):
        ...
        :returns: :class:`~ualfred.update.Version` instance
            or ``None``
    ...
    def set_last_version(self, version=None):
        ...
        :type version: :class:`~ualfred.update.Version` instance
            or ``str``
        ...
        if not version:
            if not self.version:
                self.logger.warning("Can't save last version: ualfred has no version")
                return False
    ...
    def update_available(self):
        ...
        information on how to enable your ualfred to update itself.

        :returns: ``True`` if an update is available, else ``False``

        """
        key = "__workflow_latest_version"
        # Create a new ualfred object to ensure standard serialiser
    ...
    def prereleases(self):
        """Whether ualfred should update to pre-release versions.
    ...
    def check_update(self, force=False):
        ...
        interfere in the execution of your ualfred.
    ...
    def start_update(self):
        """Check for update and download and install new ualfred file.
    ...
    def get_password(self, account, service=None):
        ...
        :param account: name of the account the password is for, e.g.
            "Pinboard"
        :type account: ``str``
        :param service: Name of the service. By default, this is the ualfred's
                        bundle ID
        :type service: ``str``
    ...
    ####################################################################
    # Methods for ualfred:* magic args
    ####################################################################
    ...
    def _register_default_magic(self):
        ...
        self.magic_arguments["delcache"] = callback(self.clear_cache, "Deleted ualfred cache")
        self.magic_arguments["deldata"] = callback(self.clear_data, "Deleted ualfred data")
        self.magic_arguments["delsettings"] = callback(self.clear_settings, "Deleted ualfred settings")
        self.magic_arguments["reset"] = callback(self.reset, "Reset ualfred")
        self.magic_arguments["openlog"] = callback(self.open_log, "Opening ualfred log file")
        self.magic_arguments["opencache"] = callback(self.open_cachedir, "Opening ualfred cache directory")
        self.magic_arguments["opendata"] = callback(self.open_datadir, "Opening ualfred data directory")
        self.magic_arguments["openworkflow"] = callback(self.open_workflowdir, "Opening ualfred directory")
        self.magic_arguments["openterm"] = callback(self.open_terminal, "Opening ualfred root directory in Terminal")
        ...
        def do_help():
            if self.help_url:
                self.open_help()
                return "Opening ualfred help URL in browser"
            else:
        ...
        def show_version():
            if self.version:
                ...
                return "This ualfred has no version number"
    ...
    def clear_cache(self, filter_func=lambda f: True):
        """Delete all files in ualfred's :attr:`cachedir`.
    ...
    def clear_data(self, filter_func=lambda f: True):
        """Delete all files in ualfred's :attr:`datadir`.
    ...
    def clear_settings(self):
        """Delete ualfred's :attr:`settings_path`."""
        if os.path.exists(self.settings_path):
            os.unlink(self.settings_path)
            self.logger.debug("deleted : %r", self.settings_path)

    def reset(self):
        """Delete ualfred settings, cache and data.
    ...
    def open_cachedir(self):
        """Open the ualfred's :attr:`cachedir` in Finder."""
        subprocess.call(["open", self.cachedir])  # nosec

    def open_datadir(self):
        """Open the ualfred's :attr:`datadir` in Finder."""
        subprocess.call(["open", self.datadir])  # nosec

    def open_workflowdir(self):
        """Open the ualfred's :attr:`workflowdir` in Finder."""
        subprocess.call(["open", self.workflowdir])  # nosec

    def open_terminal(self):
        """Open a Terminal window at ualfred's :attr:`workflowdir`."""
    ...
    def dumbify_punctuation(self, text):
        ...
        internally, but exists as a helper method for ualfred authors.
    ...
    def _load_info_plist(self):
        """Load ualfred info from ``info.plist``."""
