...
.. note::
   ...
   :class:`~ualfred.ualfred.Workflow` objects.
...
match_workflow = re.compile(r"\.alfred(\d+)?ualfred$").search
...
@total_ordering
class Download:
    """A ualfred file that is available for download.

    .. versionadded: 1.37

    Attributes:
        url (str): URL of ualfred file.
        filename (str): Filename of ualfred file.
        version (Version): Semantic version of ualfred.
    ...
    def from_releases(cls, js):
        ...
        releases containing multiple files with the same (ualfred)
    ...
    def __init__(self, url, filename, version, prerelease=False):
        ...
        Args:
            url (str): URL of ualfred file.
            filename (str): Filename of ualfred file.
            version (Version): Version of ualfred.
...
def retrieve_download(dl):
    ...
    if not match_workflow(dl.filename):
        raise ValueError("attachment not a ualfred: " + dl.filename)
...
def check_update(repo, current_version, prereleases=False, alfred_version=None):
    ...
    Args:
        repo (unicode): ``username/repo`` for ualfred's GitHub repo
        current_version (unicode): the currently installed version of the
            ualfred. :ref:`Semantic versioning <semver>` is required.
...
def install_update():
    ...
    wf().logger.info("installing updated ualfred ...")
