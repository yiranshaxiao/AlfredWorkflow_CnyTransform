...
# Save a variable to the ualfred configuration sheet/info.plist
JXA_SET_CONFIG = "Application({app}).setConfiguration({arg}, {opts});"
# Delete a variable from the ualfred configuration sheet/info.plist
JXA_UNSET_CONFIG = "Application({app}).removeConfiguration({arg}, {opts});"
# Tell Alfred to reload a ualfred from disk
...
def run_trigger(name, bundleid=None, arg=None):
    ...
    Args:
        ...
        bundleid (str, optional): Bundle ID of ualfred trigger belongs to.
...
def set_config(name, value, bundleid=None, exportable=False):
    """Set a ualfred variable in ``info.plist``.
    ...
    Args:
        ...
        bundleid (str, optional): Bundle ID of ualfred variable belongs to.
...
def unset_config(name, bundleid=None):
    """Delete a ualfred variable from ``info.plist``.
...
def reload_workflow(bundleid=None):
    ...
    Args:
        bundleid (unicode, optional): Bundle ID of ualfred to reload.
...
class uninterruptible(object):
    ...
    your ualfred is killed in the middle of critical code (e.g.
    writing data to disk), this may corrupt your ualfred's data.
